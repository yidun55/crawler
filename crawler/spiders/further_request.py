...
def frequest_51auto(response):
    car_id, = response.xpath("//a[@class='oa_ratio _compare_trigger']/@carid").extract()
...
def frequest_che168(response):
    url = response.xpath("//script/text()")\
...
def frequest_58(response):
    ...
    userid, = response.xpath("//script").re("userid:'(.*)',linkman")
    headers = {"Referer": response.url}
...
def frequest_zg2sc(response):
    shop_url = response.xpath(u"//div[@class='carfile_jxs']//a[contains(text(), '进入店铺')]/@href").extract()
    if shop_url:
        headers = {"Referer": response.url}
...
def frequest_cn2che(response):
    ...
    hidchildid = ''.join(response.xpath("//input[@id='hidchildid']/@value").extract())
    hiduserid = ''.join(response.xpath("//input[@id='hiduserid']/@value").extract())
    car_id = ''.join(response.xpath("//span[@id='carid']/text()").extract())
    hidcar_city_id = ''.join(response.xpath("//input[@id='hidcar_city_id']/@value").extract())
    params = {
